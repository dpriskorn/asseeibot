...
from asseeibot.models.crossref_engine.enums import SupportedSplit
...
class CrossrefSubject(BaseModel):
    ...
    def __lookup_in_find_match__(self,
                                 subject: str = None,
                                 split_subject: bool = False):
        """This perform one lookup and update the match_status"""
        if subject is None:
            raise ValueError("subject was None")
        self.matcher = OntologyBasedNerMatcher(crossref_subject=subject,
                                               original_subject=self.original_subject,
                                               split_subject=split_subject)
        self.matcher.lookup_subject()
        if self.matcher.match is not None:
            if self.matcher.match.approved is True:
            ...
            logger.info("Got no match from the matcher")
    ...
    def __lookup_after_split__(self, split_subject_parts: List[str]):
        ...
        if len(split_subject_parts) > 1:
            for split_subject in split_subject_parts:
                ...
                self.__lookup_in_find_match__(split_subject,
                                              split_subject=True)
    ...
    def lookup(self):
        ...
        self.__lookup_in_find_match__(subject=self.original_subject,
                                      split_subject=False)
